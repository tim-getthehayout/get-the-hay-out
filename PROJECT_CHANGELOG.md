...
| Build | File | Change |
|---|---|---|
| b20260329.2116 | HTML | Bug fix (OI-0098): `feedback` table 400 on every write. Root cause: `_feedbackRow()` sent three columns not in the Supabase schema — `resolved_at`, `confirmed_by`, `confirmed_at`. PostgREST rejected the entire upsert. These fields exist on the JS feedback object but were never added to the Supabase `feedback` table (migration script omits them). Fix: removed the three extra fields from `_feedbackRow()`. JS objects retain these fields for local functionality; they are simply not written to Supabase. |
