...
| Build | File | Change |
|---|---|---|
| b20260329.1838 | HTML | Bug fix (OI-0090): `sbSendCode()` and `sbVerifyOtp()` showed a bare "Supabase not initialised" alert when the Supabase SDK CDN script failed to load (race condition on first load after SW cache update, or CDN slow/blocked). Fix: both functions now call `sbInitClient()` on the spot before bailing — if the global became available since startup this recovers silently. If still unavailable, a `confirm()` prompt offers to reload the page, which re-fetches the CDN script. Root cause of original SDK failure: service worker `fetch` handler returns early for cross-origin requests (`if (!req.url.startsWith(self.location.origin)) return`) which can cause CDN scripts to fail on first load after a SW version switch. |
